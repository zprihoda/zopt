...
class QuadcopterAnimation():
    ...
    def _getRectangularPrism(self, center, dx, dy, dz, R=jnp.eye(3), **kwargs):
        ...
        faces = [
            [v[0], v[1], v[3], v[2]], [v[4], v[5], v[7], v[6]], [v[0], v[1], v[5], v[4]], [v[2], v[3], v[7], v[6]],
    ...
    def _addRotors(self, ax, x, R):
        ...
        center1 = center_body + R @ (
            self.armLength * jnp.array([1 / np.sqrt(2), 1 / np.sqrt(2), 0]) +
            jnp.array([0, 0, self.armWidth / 2 + self.rotorHeight / 2])
        )
