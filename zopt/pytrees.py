...
class AffineDynamics(NamedTuple):
    """
    Discrete affine dynamics tuple: (f0, f_x, f_u)
    1st order taylor series expansion: `xOut = f0 + f_x @ x + f_u @ u`
    where `f0 = f(x0,u0) - f_x @ x0 - f_u @ u0`
    """
    f0: jnp.ndarray
    f_x: jnp.ndarray
    f_u: jnp.ndarray
    ...
    def from_function(cls, dynFun: Callable[[jnp.ndarray, jnp.ndarray], jnp.ndarray], x0: jnp.ndarray, u0: jnp.ndarray):
        ...
        f0 = dynFun(x0, u0) - f_x @ x0 - f_u @ u0
        return cls(f0, f_x, f_u)
    ...
    def __call__(self, x: jnp.ndarray, u: jnp.ndarray, k: int = None):
        f0, f_x, f_u = self
        if k is None and f0.ndim != 1:
            raise ValueError("Must specify index for multi-dimensional dynamics")
        return f0 + f_x @ x + f_u @ u if k is None else self[k](x, u)
